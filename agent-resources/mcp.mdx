...
56 tools, grouped the way `?features=` groups them. The access column is the tool's own annotation, which clients use to decide what to confirm with you: **read** never changes anything, **write** only adds, **destructive** can delete or overwrite what is already there. On a read-only connection every write and destructive tool is refused.
...
<AccordionGroup>
  ...
  <Accordion title="Box — 15 tools">
    ...
    | `box_agent` | destructive | Hands a prompt to the coding agent inside the box and waits for the run; `webhook_url` returns a `run_id` at once for anything long, `json_schema` shapes the answer |
    ...
    `box_git` `clone` is what writes the GitHub credentials into the box, so a repository cloned with `git` in `box_exec` cannot `push` or `create_pr`. A clone lands at `/workspace/home/<repo>`; pass that as `folder` on later calls. A Box API key made with `box_apikey` outlives the OAuth grant that created it — revoke it from the console when it is no longer needed. `box_agent` waits for the run, and a run longer than the server's upstream timeout comes back as a timeout while it keeps going in the box: follow it with `box_runs`, or give `webhook_url` from the start. `box_env` is not a secrets store; a value set there lands in the environment of every future box.
  </Accordion>
