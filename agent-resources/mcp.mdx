...
50 tools, grouped the way `?features=` groups them. The access column is the tool's own annotation, which clients use to decide what to confirm with you: **read** never changes anything, **write** only adds, **destructive** can delete or overwrite what is already there. On a read-only connection every write and destructive tool is refused.
...
<AccordionGroup>
  ...
  <Accordion title="Box — 9 tools">
    ...
    | `box_manage` | destructive | `create`, `list`, `get`, `delete`, `pause`, `resume`, `fork` — `delete` takes one `box_id` or up to 50 `box_ids` |
  ...
  <Accordion title="Blob — 3 tools">
    ...
    | `blob_object` | destructive | `list`, `delete` — `list` pages through objects under an optional `prefix`; `delete` takes exact `paths` (up to 1,000) or a non-empty `prefix`, never both |

    Upload with the returned headers sent verbatim — they are part of the signature, and a changed or missing one is refused by storage. A `prefix` delete removes up to 5,000 objects per call and returns a `cursor` when more remain; pass it back to continue. Deleting a path that is already gone counts as deleted, and the result lists anything storage kept under `failed`. To remove a bucket, empty it with `blob_object` first. No tool returns bucket tokens. On a read-only connection `blob_upload_url`, `blob_object` `delete` and `blob_bucket` `create`/`delete` are refused; listing works.
  </Accordion>
</AccordionGroup>
