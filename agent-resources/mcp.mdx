...
<AccordionGroup>
  ...
  <Accordion title="Blob — 3 tools">
    ...
    Upload with the returned headers sent verbatim — they are part of the signature, and a changed or missing one is refused by storage. A path that is already gone counts as deleted, and the result lists anything storage kept under `failed`. To remove a bucket, empty it first: list a page, delete its paths, repeat until the list comes back empty. No tool returns bucket tokens. On a read-only connection `blob_upload_url`, `blob_object` `delete` and `blob_bucket` `create`/`delete` are refused; listing works.
  </Accordion>
</AccordionGroup>
