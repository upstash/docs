...
<Note>
  The Upstash plugin for **Claude Code, Claude Desktop, Codex, Cursor, and Gemini CLI** bundles this remote MCP server (over OAuth), so installing the plugin sets up the skills **and** the MCP in one step — no separate command. See [Install by agent](/agent-resources/clients).
</Note>
