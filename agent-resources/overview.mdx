...
Don't see your agent? See the [full list of agents](/agent-resources/clients).
...
Prefer a headless setup, or need [Upstash Box](/box/overall/quickstart) tools? Run the local server with a [Developer API key](/devops/developer-api/introduction#create-an-api-key):

```bash
npx -y @upstash/mcp-server@latest --email YOUR_EMAIL --api-key YOUR_API_KEY
```

See the [MCP server page](/agent-resources/mcp) for remote vs. local, authentication, and feature options.
