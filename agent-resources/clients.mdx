...
The plugin now bundles the **remote MCP server** (over OAuth), so this one install sets up both the skills and the MCP — approve the consent screen on the first tool call.
...
The plugin now bundles the **remote MCP server** (over OAuth), so this also connects the MCP — approve the consent screen on the first tool call.
