...
## When QStash is not the right fit

QStash is deliberately narrow. It is probably the wrong choice if:

- **You need a durable event log.** Replaying six months of events into a new
  consumer is Kafka's job, not QStash's. QStash retains logs and DLQ entries for
  days to months depending on plan.
- **You need sustained six-figure messages per second.** Kafka and Pub/Sub are
  built for that throughput profile.
- **Your consumers can't be reached over HTTP.** QStash delivers to public HTTPS
  endpoints. Services that only listen inside a VPC with no public route need a
  different transport, or a tunnel.
- **Your workflow has many dependent steps.** Chaining raw messages gets complicated.
  Use [Upstash Workflow](/workflow/getstarted), which is built on QStash and gives
  you durable, checkpointed, resumable functions.
