...
In the [serve](https://upstash-workflows.mintlify.app/qstash/workflows/basics/serve) method, you define a **Route Function** where the steps of the workflow are outlined.

These steps are created using the `WorkflowContext` object. Along with defining steps, `context` allows access to information such as the initial request payload, headers, and the current workflow run ID.

```javascript
import { serve } from "@upstash/qstash/nextjs";

export const POST = serve<string>(
  // The route function receives a WorkflowContext as a parameter:
  async (context) => {
...
This page documents the fields and methods available in `WorkflowContext`.

## Fields

**context.qstashClient**: Provides access to a QStash client, which is also used by the serve method. This can be overridden by passing the [qstashClient](/qstash/workflow/basics/serve#qstashclient) parameter in serve.

**context.workflowRunId**: The run ID of the current workflow.

**context.url**: The URL where the workflow endpoint is hosted. This can be overridden with the [url](/qstash/workflow/basics/serve#url) parameter or updated with the [baseUrl](/qstash/workflow/basics/serve#baseUrl) parameter.

**context.failureUrl**: The URL to be called in case of workflow failure. This can be configured through the [failureUrl](/qstash/workflow/basics/serve#failureurl) or [failureFunction](/qstash/workflow/basics/serve#failurefunction) parameters.

**context.requestPayload**: The payload received in the request that initiated the workflow run. Its type can be set through [serve](https://upstash-workflows.mintlify.app/qstash/workflows/basics/serve).

**context.rawInitialPayload**: The payload received in the request that initiated the workflow run, provided as a string.

**context.headers**: The headers received in the request that initiated the workflow run, provided as a `Headers` object.

**context.env**: Environment variables from `process.env` if it exists. Can be overwritten with [env]((/qstash/workflows/basics/serve#env)) option in serve.
...
If `context.run` throws an exception, the SDK automatically handles it by retrying the `context.run` operation three times with exponentially increasing delays, as [explained here](/qstash/features/retry). This behavior will be configurable in [upcoming releases](/qstash/workflow/roadmap).
...
If `context.call` returns a non-success status code (anything other than 2xx), the SDK automatically retries the call three times with exponentially increasing delays, as [explained here](/qstash/features/retry). This behavior will be configurable in [upcoming releases](/qstash/workflow/roadmap).
