...
In the [serve](/qstash/workflow/basics/serve) method, you define a **Route Function** where the steps of the workflow are outlined.
...
**context.requestPayload**: The payload received in the request that initiated the workflow run. Its type can be set through [serve](/qstash/workflow/basics/serve).
...
**context.env**: Environment variables from `process.env` if it exists. Can be overwritten with [env](/qstash/workflow/basics/serve#env) option in serve.
