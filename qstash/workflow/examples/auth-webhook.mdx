...
export const POST = serve<UserCreatedPayload>(async (context) => {
  ...
  await context.run("sync user", async () => {
    await createUserInDatabase({ name, email });
  });

  await context.run("create new user in stripe", async () => {
    await createNewUserInStripe();
  });

  await context.run("start trial in Stripe", async () => {
    await startTrialInStripe();
  });
  ...
  const stats = await context.run("get user stats", async () => {
    return await getUserStats(email);
  });
...
async function createNewUserInStripe() {
  // Implement logic to create a new user in Stripe
  console.log("Creating a user in Stripe");
}

async function startTrialInStripe() {
  // Implement logic to start a trial in Stripe
  console.log("Starting a trial of 14 days in Stripe");
}

async function getUserStats(email: string) {
  // Implement logic to get user stats
  console.log("Getting user stats for", email);
  return {
