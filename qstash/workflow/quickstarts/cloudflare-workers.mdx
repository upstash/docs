...
For QStash, a tool that's used to run workflows under the hood, we need to provide a publically accessible URL. You can easily forward your localhost URL to a live URL by following our guide about [setting up your workflow endpoint for local development](/qstash/workflow/howto/local-development).
