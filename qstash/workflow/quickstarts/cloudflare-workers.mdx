...
export default {
  fetch(req: Request, env: Env, ctx: ExecutionContext) {
    // 👇 For TypeScript users: define initial payload type
...
In production, a workflow `baseUrl` is automatically set and can be omitted.

Once you have a publically accessible URL for your locally running worker, trigger your workflow by sending a POST request. For each workflow run, a unique workflow run ID is returned:
