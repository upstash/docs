...
On October 18, 2024 we released a new sdk `@upstash/workflow` for Upstash Workflow. Previously,
`@upstash/qstash` was used. `@upstash/qstash` will continue being developed alongside QStash,
but future Upstash Workflow development will take place in `@upstash/workflow`. Workflow
imports will be removed from `@upstash/qstash` in a future release.
...
Firstly, you will need to change the imports from `@upstash/qstash` to @upstash/workflow:
