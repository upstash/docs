...
{/* To take certain action whenever a workflow fails (i.e. cleaning up your database or logging to Sentry), configure a `failureFunction` or a `failureUrl` for your workflow.  
...
Note: If you use a custom authorization method to secure your workflow endpoint, add authorization to the `failureFunction` too. Otherwise, anyone could invoke your failure function. Read more here: [securing your workflow endpoint](/qstash/workflow/howto/security).
