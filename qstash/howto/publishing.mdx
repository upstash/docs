...
You can redact the message body and/or specific headers from QStash logs to prevent sensitive data like authorization tokens or API keys from being stored in logs. Use the `redact` parameter in the SDK or the `Upstash-Redact-Fields` header when using the REST API directly.
...
curl -XPOST \
    -H 'Authorization: Bearer XXX' \
    -H 'Upstash-Forward-Authorization: Bearer token' \
    -H 'Upstash-Redact-Fields: body,header[Authorization]' \
    -H "Content-type: application/json" \
    -d '{ "secret": "sensitive-data" }' \
    'https://qstash.upstash.io/v2/publish/https://example.com'
```
```typescript TypeScript
import { Client } from "@upstash/qstash";
...
The `Upstash-Redact-Fields` header accepts a comma-separated list of redaction targets:
- `body` - Redacts the request body
- `header` - Redacts all headers
- `header[HeaderName]` - Redacts a specific header (e.g., `header[Authorization]`)

You can combine multiple values: `body,header[Authorization],header[X-API-Key]`
