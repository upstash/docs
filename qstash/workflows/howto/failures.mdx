...
-  Steps in your workflow might also fail if they exceed the [platform's time limitations](/qstash/workflows/platforms/platforms). When a step takes too long to execute, it can time out, causing the workflow to fail.
...
When a workflow failed, a related event will be fired to [Dead Letter Queue](/qstash/howto/handling-failures#dead-letter-queue)
