...
Copy the `Forwarding` URL from ngrok and use it as the `baseUrl` parameter in the next section, appending the path to your API (e.g., `/api/example`).
...
In this setup, we'll use the [baseUrl](/qstash/workflows/basics/serve#baseUrl-option) parameter in the `serve` method to override the default URL. Without this, the SDK will report its URL as `localhost`, preventing QStash from accessing the endpoint. Note that setting the `url` is only necessary for local development.
...
export const POST = serve<string>(
  ...
  { baseUrl: "https://d2ed-176-236-134-194.ngrok-free.app" }
);
```
