...
Set the `UPSTASH_WORKFLOW_URL` environment variable to the `Forwarding` URL from ngrok. Without this, the SDK will report its URL as `localhost`, preventing QStash from accessing the endpoint. Note that setting the `UPSTASH_WORKFLOW_URL` is only necessary for local development. An alternative to `UPSTASH_WORKFLOW_URL` is to use [the `baseUrl` parameter](/qstash/workflows/basics/serve#baseurl).
...
```bash 
curl -X POST https://d2ed-176-236-134-194.ngrok-free.app/api/example -d 'testData'
```
