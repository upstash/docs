...
In this setup, we'll use the [baseUrl](/qstash/workflows/basics/serve#baseUrl) parameter in the `serve` method to override the default URL. Without this, the SDK will report its URL as `localhost`, preventing QStash from accessing the endpoint. Note that setting the `baseUrl` is only necessary for local development.
