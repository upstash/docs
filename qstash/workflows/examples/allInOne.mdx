...
import { sendEmail, reportFailure, getUserEmail } from "./utils"

export const POST = serve<{ userId: string, question: string }>(async context => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    // sleep for 1 hour
    await context.sleep("sleep for email retry", 24 * 60 * 60);
  }
...
HTTP calls to third-party services are non-blocking and do not consume the endpoint's execution time when using [context.call](/qstash/workflows/basics/context#context-call-method).
...
const openAIPromise = context.call(
  ...
  "POST", 
  {
    ...
    messages: [
      { role: "system", content: "You are a helpful assistant." },
      { role: "user", content: context.requestPayload.question }
    ]
  },
  {
    authorization: `Bearer ${process.env.OPENAI_API_KEY}`,
...
[Sleeps](/qstash/workflows/basics/context#context-sleep-method) also do not consume the endpoint's execution time, allowing you to efficiently manage delays.
