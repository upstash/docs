...
**context.qstashClient**: Provides access to a QStash client, which is also used by the serve method. This can be overridden by passing the [qstashClient](/qstash/workflows/basics/serve#qstashclient-option) parameter in serve.

**context.workflowRunId**: The run ID of the current workflow.

**context.url**: The URL where the workflow endpoint is hosted. This can be overridden with the [url](/qstash/workflows/basics/serve#url-option) parameter or updated with the [baseUrl](/qstash/workflows/basics/serve#baseUrl-option) parameter.

**context.failureUrl**: The URL to be called in case of workflow failure. This can be configured through the [failureUrl](/qstash/workflows/basics/serve#failureurl-option) or [failureFunction](/qstash/workflows/basics/serve#failurefunction-option) parameters.
