...
export const POST = serve<string>(
  routeFunction: async (context) => {
    ...
    var result1 = await context.run("step1", async () => {
      result1 = await someWork(input);
    });
...
It is not allowed to call any of the `context` methods (such as `call`, `sleep`, `sleepFor`, or `run`) within the function passed to `context.run`. Doing so results in undefined behavior.
