...
Besides Next.js, the `serve` method supports multiple frameworks and platforms. See [all supported platforms](/qstash/workflows/platforms/platforms) here.
...
Note that if you are using a [custom authorization method](/qstash/workflows/howto/security#custom-authorization-method), you should also add authentication to `failureFunction`.
...
You can set the `UPSTASH_WORKFLOW_URL` environment variable to apply `baseUrl` across your entire application.

Setting this environment variable is especially useful during [local development](/qstash/workflows/howto/local-development).
