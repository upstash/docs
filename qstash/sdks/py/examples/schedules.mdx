...
#### Create a schedule with redacted fields

You can redact the message body and/or specific headers from QStash logs using the `redact` parameter.
...
schedule_id = client.schedule.create(
    destination="https://my-api...",
    cron="*/5 * * * *",
    body=json.dumps({"secret": "sensitive-data"}),
