...
You can redact the message body and/or specific headers from QStash logs using the `redact` parameter. This is useful for preventing sensitive data like authorization tokens or API keys from being stored in logs.
...
// Redact both body and specific headers
await client.publishJSON({
  url: "https://my-api...",
  body: { secret: "sensitive-data" },
  headers: { "Authorization": "Bearer token" },
