...
As a result of that, a few endpoints have changed and we also added new one.
...
To migrate to v2, all you need to do is call the new [`/v2` endpoint](/qstash/api-reference).
...
Instead of creating schedules through the `/v1/publish` endpoint, there is a dedicated `/v2/schedules` api now.
