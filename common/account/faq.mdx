...
You can delete your credit card from `Account` > `Billing` page. If it is the last card on the account, deleting it moves the account to the free plan: any outstanding usage is charged to the card first, then your resources are downgraded to the free plan. This is only possible if your resources fit within the free plan limits, such as the number of databases, data size and monthly bandwidth; otherwise the console tells you what to reduce first. For teams, only owners and admins can delete the last card.
