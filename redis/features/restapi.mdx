...
<Note>
  The [Vercel integration](/redis/howto/vercelintegration) uses different
  variable names for each setup. Linking an existing Upstash account creates
  `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN`.
  A Vercel-managed Upstash account uses `KV_REST_API_URL` and
  `KV_REST_API_TOKEN` for the same credentials.
</Note>
...
The Redis [`INFO`](/redis/commands/server/info) command works over REST with a
database token. The default HTTP response is JSON: its `result` field contains
the CRLF-delimited `INFO` text. Use `jq -r` to print that field:
...
Fields such as `used_memory`, `connected_clients`, `total_commands_processed`,
and `instantaneous_ops_per_sec` describe database load and command activity.
They are not a remaining monthly quota or a billing total. Use the console's
[usage charts](/redis/howto/metrics-and-charts) to monitor plan usage and limits.

The Free plan includes 500,000 commands per month. If you reach the command
limit, requests fail with `ERR max requests limit exceeded`; see
[Maximum requests limit](/redis/troubleshooting/max_requests_limit).
REST and Redis protocol use the same command pricing; see
[Pricing & Limits](/redis/overall/billing).
