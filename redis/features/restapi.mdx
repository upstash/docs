...
database token, so you can inspect the database backing your API without
...
The reply is the standard CRLF-delimited `INFO` text report. The `memory`,
`clients` and `stats` sections include `used_memory`, `connected_clients`,
`total_commands_processed` and `instantaneous_ops_per_sec`, which summarize
current load and past command volume. Plan limits appear as errors when
exceeded: for example, free databases fail with
`ERR max daily request limit exceeded` (see
[troubleshooting](/redis/troubleshooting/max_daily_request_limit)). Billing is
per command, identically for REST and Redis protocol — see
