...
<Note>
  If you connected your database through the
  [Vercel integration](/redis/tutorials/nextjs_with_redis),
  the same values are injected as `KV_REST_API_URL` and `KV_REST_API_TOKEN`
  instead of `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN`.
</Note>
