...
<Warning>
  Resetting the password (the **Reset Credentials** button in the console)
  immediately revokes the current password and every REST token — _Standard_
  and _Read Only_ — since REST tokens are derived from the password. Update the
  new token in your environment variables and redeploy dependent applications,
  or they will start failing with `401 Unauthorized`.
</Warning>
