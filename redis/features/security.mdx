...
[IP allowlisting](/redis/howto/ipallowlist) restricts database access to the
IPv4 addresses or CIDR ranges that you permit. It is available on all paid
plans. Clients need known outbound IP addresses; serverless clients with dynamic
egress addresses need a static egress configuration before you can allowlist them.
...
VPC Peering requires an [Enterprise contract](/redis/overall/enterprise#vpc-peering-and-private-links).

## Private Link

AWS Private link provides private connectivity between Upstash Database and your
Redis client inside AWS infrastructure. Private Link requires an [Enterprise contract](/redis/overall/enterprise#vpc-peering-and-private-links).

## Security Features by Plan

| Feature                         | Free | Pay-As-You-Go | Fixed | Enterprise |
| ------------------------------- | ---- | ------------- | ----------- | ---------- |
| TLS                             | ✅   | ✅            | ✅          | ✅         |
| IP allowlisting                 | –    | ✅            | ✅          | ✅         |
| Redis ACL                       | –    | ✅            | ✅          | ✅         |
...
| Encryption at rest              | –    | ✅*           | ✅*         | ✅         |
| Backup retention up to 3 days   | –    | ✅*           | ✅*         | ✅         |

<sub>*With the [Prod Pack](/redis/overall/enterprise#prod-pack-features)
add-on, available on pay-as-you-go and fixed-price plans. Enterprise also
offers custom backup configurations on request. See [Pricing &
Billing](/redis/overall/billing) for plan details.</sub>
