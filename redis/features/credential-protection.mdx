...
<Frame>
  <img src="/img/credential-protection/activate-protect-credentials.png" alt="Enable Redis credential protection" />
</Frame>

3. Save the credentials shown in the modal:

<Frame>
  <img src="/img/credential-protection/save-credentials.png" alt="Save protected Redis credentials" />
</Frame>
...
Resetting the credentials immediately revokes the current ones — the password and both the _Standard_ and _Read Only_ REST tokens stop working at once, since the tokens are derived from the password. Update the new credentials in your environment variables and redeploy every application that uses the database.

<Frame>
  <img src="/img/credential-protection/reset-credentials.png" alt="Reset protected Redis credentials" />
</Frame>
