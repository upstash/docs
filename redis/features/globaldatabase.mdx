...
Lua scripts follow the same routing: [`EVAL`](/redis/commands/scripting/eval) and
[`EVALSHA`](/redis/commands/scripting/evalsha) always execute on the Primary
Region, while [`EVAL_RO`](/redis/commands/scripting/eval-ro) and
[`EVALSHA_RO`](/redis/commands/scripting/evalsha-ro) are served by the nearest
Read Region. Because the script in an `EVAL_RO` call cannot write, declare
read-only scripts with `EVAL_RO` so that reads keep being served close to the
client.
...
database. After creation you can only add or remove read regions on a running
database; the primary region cannot be changed. A regional database also cannot
be upgraded in place to a global one (see
[Upgrade from Regional to Global](#upgrade-from-regional-to-global)).
