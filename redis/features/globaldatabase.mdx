...
Use [`EVAL_RO`](/redis/commands/scripting/eval-ro) or
[`EVALSHA_RO`](/redis/commands/scripting/evalsha-ro) for scripts that only read
data. These commands reject scripts that try to write.
