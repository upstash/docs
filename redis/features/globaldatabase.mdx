...
<Frame><img src="/img/globaldb/regionselect.png" width="520" /></Frame>
...
<Frame><img src="/img/globaldb/map2.png" width="1000" /></Frame>
...
<Note>
    Global Database is designed to optimize the latency of READ operations. It
    may not be a good choice if your use case is WRITE heavy.
</Note>
...
Global Database charges $0.4 per 100K commands. The write commands are
replicated to all read regions in addition to primary region so the replications
are counted as commands. For example, if you have 1 primary 1 read region, 100K
writes will cost $0.8 ($0.4 x 2). You can use Global Database in the free tier
too. Free usage is limited with max one read region.
