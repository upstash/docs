...
- Added [Vector](/redis/commands/vector/overview) feature for storing embeddings and running nearest-neighbor queries directly in Redis.
- Added [Array commands](/redis/commands/array/overview) for sparse, index-addressed data, including range operations, scans, and fixed-size rings.
- Expanded [Redis Search](/redis/search/introduction) with [searchable Redis streams](/redis/search/streams), multi-value text and keyword fields, and [composite aggregations](/redis/search/aggregation-operators/bucket-aggregations/composite).
- Increased Redis compatibility with [`MSETEX`](/redis/commands/string/msetex), [`SDIFFCARD`](/redis/commands/set/sdiffcard), and [`SUNIONCARD`](/redis/commands/set/sunioncard).
</Update>
