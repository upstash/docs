...
### ACL
Access Control List (ACL) is a security model that manages database access. You can create multiple users with different roles to control their actions on your databases.

We recommend using ACL if your database is accessible to multiple developers.

### Multi-Zone High Availability

With Prod Pack add-on, read regions of your database are deployed with [multi-zone high availability](/redis/features/replication#high-availability). If one replica or availability zone fails, another replica in a different zone continues serving traffic in the same region. This ensures that your database remains available without any additional latency.
