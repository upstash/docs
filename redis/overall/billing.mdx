...
support larger data sizes and custom limits — [contact us](/common/help/support).
See [Security Features by Plan](/redis/features/security#security-features-by-plan)
for which security features each plan includes.</sub>
...
- **What counts as a command.** Pricing is based on the number of commands (requests)
  ...
  commands made over the Redis protocol. Operational commands such as `INFO`,
  `PING`, and `AUTH` are not charged. See the
  [pricing FAQ](https://upstash.com/pricing/redis#faq) for the full list of exceptions.
- **Monitoring usage.** Use the [console usage charts](/redis/howto/metrics-and-charts)
  for billing and plan usage. [`INFO`](/redis/commands/server/info) reports database
  activity, not a billing total or remaining monthly quota. See
  [Metrics and Monitoring](/redis/features/restapi#metrics-and-monitoring).
