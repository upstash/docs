...
function dailyActiveUsers(day = today()) {
  return redis.bitcount(`active:${day}`);
}
...
async function weeklyActiveUsers(days: string[]) {
  await redis.bitop("OR", "active:week", ...days.map((d) => `active:${d}`));
  return redis.bitcount("active:week");
}
...
async function retained(dayA: string, dayB: string) {
  ...
  return redis.bitcount("tmp:retained");
}
```
...
[querying](/redis/search/querying) and [aggregating](/redis/search/aggregating)
