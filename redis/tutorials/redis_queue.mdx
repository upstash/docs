...
Polling an empty queue in a tight loop wastes CPU and network calls. Blocking pop
commands let a consumer sleep until a task arrives. `BRPOP` blocks on the right end
of the list and returns as soon as an item is available, or after a timeout:

```ts
// Wait up to 5 seconds for a task. Use 0 to block indefinitely.
const result = await redis.brpop("tasks", 5);
// result is [queueName, task] or null on timeout
```

<Note>
Blocking commands hold the connection open for the duration of the wait. This suits
long-running workers, but in short-lived serverless functions you usually want a
short timeout (or plain `RPOP` polling) so the function doesn't hang on an idle
queue. For event-driven serverless delivery, consider
[Upstash QStash](/qstash/overall/getstarted), which pushes messages to your HTTP
endpoint instead.
</Note>
...
async function enqueue(job: Job, delayMs = 0, priority = 0) {
...
async function claimDueJobs(limit = 10): Promise<string[]> {
...
async function processJob(raw: string) {
  const job = JSON.parse(raw) as Job & { attempts?: number };
  ...
  try {
    ...
    if (attempts + 1 >= MAX_ATTEMPTS) {
      // Out of retries: set it aside for inspection
      await redis.lpush(
        "jobs:dead-letter",
        JSON.stringify({ job, error: String(err), failedAt: Date.now() }),
      );
    } else {
