---
slug: auto_complete_with_serverless_redis
title: Autocomplete API with Serverless Redis
sidebar_label: Autocomplete API
---
import IORedisNote from '../howto/ioredisnote.md'

This tutorial implements an autocomplete API powered by serverless Redis. See [the demo](https://auto-complete-example.vercel.app/) and [API endpoint](https://wfgz7cju24.execute-api.us-east-1.amazonaws.com/query?term=ca) and [the source code](https://github.com/upstash/serverless-tutorials/tree/master/auto-complete-api).  
                                                                      
We will keep country names in a Redis Sorted set. In Redis sorted set, elements with the same score are sorted lexicographically. So in our case, all country names will have the same score, 0. We keep all prefixes of country and use ZRANK to find the terms to suggest. See [this blog post](http://oldblog.antirez.com/post/autocomplete-with-redis.html) for the details of the algorithm. 


### Step 1: Project Setup
:::note
I will use Serverless framework for this tutorial. You can also use [AWS SAM](using_aws_sam.md)
:::
...
Edit handler.js file as below.
See [the blog post](http://oldblog.antirez.com/post/autocomplete-with-redis.html) for the details of the algorithm.
...
If you do not have one, create a database following this [guide](../overall/getstarted.md). Copy the Redis URL by clicking `Redis Connect` button inside database page. Copy the URL for ioredis as we use ioredis in our application. Create .env file and paste your Redis URL:
```text
...
We will initialize the database with country names. Copy and run initdb.js script from [here](https://github.com/upstash/serverless-tutorials/blob/master/auto-complete-api/initdb.js). 
