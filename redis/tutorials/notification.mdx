...
sidebar_label: Serverless Notification API
...
Check out [the code here](https://github.com/upstash/examples/tree/master/serverless-notification-api).
...
Create a free database from [Upstash](https://console.upstash.com/) and copy the REST url and read only token. You should switch the Read-Only Token setting. In the database details page, click on the `Read-Only Token` switch.
            ...
            try {
                ...
                const response = await fetch(
                        ...
                        "/WITHSCORES/LIMIT/0/1",
                    {
                        headers: {
                            Authorization:
                                "Bearer REPLACE_UPSTASH_REDIS_REST_TOKEN",
                        },
                    }
                );
...
You can also remove a message using the [ZREM](https://redis.io/commands/zrem) command.
