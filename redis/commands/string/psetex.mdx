...
<Warning>
  Prefer [`SET`](/redis/commands/string/set) with the `PX` argument in new code: `SET <key> <value> PX <milliseconds>`. `PSETEX` is deprecated in Redis but still supported by Upstash.
</Warning>
