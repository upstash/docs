...
<Warning>
  Prefer [`BLMOVE`](/redis/commands/list/blmove) with `RIGHT` and `LEFT` in new code: `BLMOVE <source> <destination> RIGHT LEFT <timeout>`. `BRPOPLPUSH` is deprecated in Redis but still supported by Upstash.
</Warning>
