...
<Warning>
  Prefer [`LMOVE`](/redis/commands/list/lmove) with `RIGHT` and `LEFT` in new code: `LMOVE <source> <destination> RIGHT LEFT`. `RPOPLPUSH` is deprecated in Redis but still supported by Upstash.
</Warning>
