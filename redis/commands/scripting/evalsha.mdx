...
Locking behaviour comes from the cached script body, not from the call: the script takes the global lock unless its shebang sets the `allow-key-locking` flag, in which case the [hash tag](/redis/features/key-locking#hash-tags) of each key passed in `KEYS` is locked. Because the flag lives in the body, changing it means loading a new script and calling the new digest. See [Key-Based Locking](/redis/features/key-locking).
...
curl -X POST "$UPSTASH_REDIS_REST_URL" \
  ...
  -d '["SCRIPT", "LOAD", "return ARGV[1]"]'

# Run the cached script.
curl -X POST "$UPSTASH_REDIS_REST_URL" \
  -H "Authorization: Bearer $UPSTASH_REDIS_REST_TOKEN" \
  -H "Content-Type: application/json" \
  -d '["EVALSHA", "098e0f0d1448c0a81dafe820f66d460eb09263da", 0, "hello"]'
```

The second response is `{ "result": "hello" }`. If the script is no longer
cached, the command returns a `NOSCRIPT` error; reload it before retrying.
