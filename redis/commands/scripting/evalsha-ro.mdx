...
Read-only scripts take the global lock like any other script unless the cached body's shebang sets the `allow-key-locking` flag, for example `#!lua flags=no-writes,allow-key-locking`. With the flag, the call takes shared read locks on the [hash tag](/redis/features/key-locking#hash-tags) of each key passed in `KEYS`. See [Key-Based Locking](/redis/features/key-locking).
...
curl -X POST "$UPSTASH_REDIS_REST_URL" \
  -H "Authorization: Bearer $UPSTASH_REDIS_REST_TOKEN" \
  -H "Content-Type: application/json" \
  -d '["EVALSHA_RO", "098e0f0d1448c0a81dafe820f66d460eb09263da", 0, "hello"]'
```
