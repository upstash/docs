...
<Note>
  Vector indexes are an Upstash extension. They are not part of open-source Redis: the Upstash SDKs have typed helpers for them, and other client libraries reach them through their generic command interface. They are also separate from [Upstash Vector](/vector/overall/getstarted), which is a standalone vector database with its own API and SDKs.
</Note>
...
The Upstash SDKs expose vector indexes through a `vector` namespace that returns a handle to an index.
...
// or, for an existing index: const index = redis.vector.index("docs");

await index.add("doc-1", [0.1, 0.2, 0.3]); // number[], Float32Array, or { base64 }
const hits = await index.query({ vector: [0.1, 0.2, 0.3], topK: 5 }); // [{ id, score }]
```
</Tab>
<Tab title="Python">
```python
...
index.add("doc-1", [0.1, 0.2, 0.3])  # list of numbers, FP32 bytes, or a base64 string
hits = index.query(vector=[0.1, 0.2, 0.3], top_k=5)  # [VectorQueryResult(id, score)]
```
</Tab>
</Tabs>

Other client libraries send them through the generic command interface: `redis.call("VECTOR.QUERY", ...)` in ioredis, `client.sendCommand(["VECTOR.QUERY", ...])` in node-redis, `client.execute_command("VECTOR.QUERY", ...)` in redis-py, `client.Do(ctx, "VECTOR.QUERY", ...)` in go-redis, and `redis::cmd("VECTOR.QUERY")` in redis-rs.
