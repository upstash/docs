...
- Pass every key the function reads in the key list when possible. With `allow-key-locking`, a dynamic key is accepted only when its [hash tag](/redis/features/key-locking#hash-tags) is already locked, and a cold dynamic key is loaded with that lock held. See [Dynamic Keys and Latency](/redis/features/key-locking#dynamic-keys-and-latency).
