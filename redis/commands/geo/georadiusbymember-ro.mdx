...
<Warning>
  Prefer [`GEOSEARCH`](/redis/commands/geo/geosearch) with `FROMMEMBER` and `BYRADIUS` in new code: `GEOSEARCH <key> FROMMEMBER <member> BYRADIUS <radius> (m | km | ft | mi)`. `GEORADIUSBYMEMBER_RO` is deprecated in Redis but still supported by Upstash.
</Warning>
