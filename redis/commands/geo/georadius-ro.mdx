...
<Warning>
  Prefer [`GEOSEARCH`](/redis/commands/geo/geosearch) with `FROMLONLAT` and `BYRADIUS` in new code: `GEOSEARCH <key> FROMLONLAT <longitude> <latitude> BYRADIUS <radius> (m | km | ft | mi)`. `GEORADIUS_RO` is deprecated in Redis but still supported by Upstash.
</Warning>
