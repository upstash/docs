...
<AccordionGroup>
  ...
  <Accordion title="Security: network egress is deny-all by default">
    ...
    Network policies accept plain domain/CIDR allow-lists only. Eve's per-domain firewall rules
    (`transform` header injection, `forwardURL`) have no Box equivalent, so the backend **throws** if you
    pass them rather than silently dropping them. To broker credentials onto outbound requests, set
    Box's `attachHeaders` at backend creation: `upstash({ attachHeaders: { "api.example.com": { ... } } })`.
  </Accordion>
