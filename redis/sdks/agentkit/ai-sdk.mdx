...
`@upstash/agentkit-ai-sdk` is the [Vercel AI SDK](https://ai-sdk.dev) adapter. It adds chat history,
agent memory, Redis-Search tools, rate limiting, and tool caching to `generateText` / `streamText`.
`redis` defaults to `Redis.fromEnv()`, so you import only from this package.
