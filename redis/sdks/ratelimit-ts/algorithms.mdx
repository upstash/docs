...
<Tip>
  Rate-limiting divides time into fixed chunks (windows). For example, with a 24-hour period, a set number of requests (e.g., 2) are allowed per day, with fixed boundaries. However, the reset time is not necessarily 24 hours from the first request. Instead, it is the end of the current time chunk the request falls into. This is the same for fixed window and sliding window algorithms.
</Tip>
