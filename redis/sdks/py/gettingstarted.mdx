...
By default, `upstash-redis` retries a request once, 3 seconds after a failure.
You can customize this when creating the client with `rest_retries` and
`rest_retry_interval` (in seconds):
...
See [Features](/redis/sdks/py/features#retry-mechanism) for more information.
