...
The client retries failed requests once by default. To customize the retries,
pass `rest_retries` and `rest_retry_interval` (seconds) to the client, e.g.
`Redis.from_env(rest_retries=3, rest_retry_interval=2)`. See the [Retry
mechanism](/redis/sdks/py/features#retry-mechanism) section for details.
