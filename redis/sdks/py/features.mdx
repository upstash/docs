...
```python
redis = Redis(url="UPSTASH_REDIS_REST_URL", token="UPSTASH_REDIS_REST_TOKEN", rest_retries=3, rest_retry_interval=2)
```

For the retry behavior of the TypeScript SDK, see
[Retries](/redis/sdks/ts/retries).
