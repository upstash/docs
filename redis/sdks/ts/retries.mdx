...
By default `@upstash/redis` will retry sending your request when network errors
...
The Python SDK exposes retry settings through the `rest_retries` and
`rest_retry_interval` constructor options; see the [Retry
mechanism](/redis/sdks/py/features#retry-mechanism) section in the Python
documentation.
