...
The client retries failed requests automatically (5 retries with an exponential
backoff). You can set a custom retry policy through the `retry` option in the
`Redis` constructor — see [Retries](/redis/sdks/ts/retries).
