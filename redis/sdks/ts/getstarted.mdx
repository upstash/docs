...
By default `@upstash/redis` retries failed requests 5 times with a backoff of
`(retryCount) => Math.exp(retryCount) * 50` milliseconds. You can customize this
in the `Redis` constructor:
...
See [Retries](/redis/sdks/ts/retries) for the full list of retry options.
