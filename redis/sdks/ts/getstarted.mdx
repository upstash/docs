...
By default, `@upstash/redis` retries requests 5 times when network errors occur,
with a backoff of `Math.exp(retryCount) * 50` milliseconds. Set the `retry`
option to use a different policy:
...
environment. See [Retries](/redis/sdks/ts/retries) for details.
