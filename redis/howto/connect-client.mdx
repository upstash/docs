...
To customize the retry behavior, pass a `retry` option to the constructor:
...
See [Retries](/redis/sdks/ts/retries) for all configurable retry options and the default values.
