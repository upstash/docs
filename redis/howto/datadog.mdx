...
Metrics arrive in Datadog under `upstash.db.<metric>` with `team` and `database` tags, refreshed every few minutes. Latencies are in nanoseconds over the last ten minutes.
...
| `lualatencyp99` | Lua script execution latency |
| `throttledcommands` | Commands slowed down by the rate limit today |
| `authfailures` | Failed authentications per second as seen by the server. Bad credentials are rejected at the proxy, so this stays 0 for proxied databases |
