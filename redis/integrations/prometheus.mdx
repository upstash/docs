...
| `auth_failures`, `rest_auth_failures` | Failed authentications the server saw over TCP and REST since start. Bad credentials are rejected at the proxy, so these stay 0 for proxied databases |
