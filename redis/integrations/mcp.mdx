...
- **SSE**: Designed for server deployments. However, since clients don't yet support direct SSE connections, you need a proxy server. The proxy acts as a `stdio` server for the client and communicates with the SSE server in the background.
