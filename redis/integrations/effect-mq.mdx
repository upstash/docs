...
```bash
npm install effect-mq effect@4.0.0-rc @effect/platform-node redis
```

`effect-mq` targets Effect v4. The Redis store does not bundle a client; on Node.js it uses `node-redis` through `@effect/platform-node`. On Bun, install `@effect/platform-bun` instead and use `BunRedis` (no extra client needed).

## Usage

Create the Redis store layer with your Upstash connection string. The URL must start with `rediss://` since Upstash requires TLS:
...
<Note>
  effect-mq's Redis store uses plain-prefixed keys without hash tags, so it does not support Redis Cluster. Upstash Redis databases are cluster-unaware from the client's point of view, so this is not an issue.
</Note>
