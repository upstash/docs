...
Use `$contains` for search-as-you-type functionality:
...
```bash
# As user types "ja"
SEARCH.QUERY users '{"profile.displayName": {"$contains": "ja"}}' LIMIT 5

# As user types "jane sm"
SEARCH.QUERY users '{"profile.displayName": {"$contains": "jane sm"}}' LIMIT 5
```
</Tab>
...
- Use `$contains` for search-as-you-type autocomplete functionality
