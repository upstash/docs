...
`TEXT`, `KEYWORD`, `U64`, `I64`, `F64`, and `DATE` fields can contain multiple values. A condition
matches the document when at least one array element matches it. The query value remains a single
value; you do not need to search the whole stored array.
...
{
  "tags": ["new", "sale"],
  "descriptions": ["Wireless headphones", "Includes a travel case"],
  "prices": [199.99, 179.99]
}
```

each query below matches it:
...
await products.query({
  filter: { prices: { $lt: 180 } },
});

await products.query({
  filter: { descriptions: { $phrase: "wireless headphones" } },
});
```
</Tab>
...
<Tab title="Python">
```python
products.query(filter={"tags": {"$eq": "sale"}})

products.query(filter={"prices": {"$lt": 180}})

products.query(filter={"descriptions": {"$phrase": "wireless headphones"}})
```
</Tab>
...
SEARCH.QUERY products '{"prices":{"$lt":180}}'

SEARCH.QUERY products '{"descriptions":{"$phrase":"wireless headphones"}}'
```
</Tab>

</Tabs>

For `TEXT`, phrase matching stays within one array element. For example, `"headphones includes"`
does not match the document above because those words are in different elements.

Search results keep the field's stored array unchanged. When you use `HIGHLIGHT` on a multi-value
`TEXT` field, only matching string elements are marked up in the response. Other elements keep their
original values, and highlighting does not change the stored document.

See [Multi-value fields in schema definitions](/redis/search/schema-definition#multi-value-fields)
for supported types and the required storage format for JSON, string, hash, and stream indexes.
...
For JSON and string indexes, that means the stored JSON objects as a whole. For hash indexes, it
means all fields and values. For stream indexes, the document key is the stream entry ID and the
content contains that entry's field-value pairs.
