...
| [`$sum`](./aggregation-operators/metric-aggregations/sum) | Total, with an option to return `null` when no values are collected |
...
By default, `$sum` returns `0` when no values are collected — either no documents match, or the matching documents all have a missing/null value for the field and no `missing` fallback is set. Set `nullIfNoMatch: true` to return `null` for that case. This lets you distinguish it from a real sum of zero.
...
| [`$composite`](./aggregation-operators/bucket-aggregations/composite) | Paginated combinations of several sources |
