...
const products = await redis.search.createIndex({
  ...
  schema: s.object({
    ...
    description: s.string(),
    descriptionExact: s.string().noStem().as("description"),

    // Create a short alias for a deeply nested field
    authorName: s.string().as("metadata.author.displayName"),
  }),
});
```
</Tab>
...
# Create a short alias for a deeply nested field
SEARCH.CREATE products ON JSON PREFIX 1 product: SCHEMA description TEXT descriptionExact TEXT NOSTEM AS description authorName TEXT AS metadata.author.displayName
```
</Tab>
