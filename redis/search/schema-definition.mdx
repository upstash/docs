...
KEYWORD fields support numeric query operators (`$eq`, `$ne`, `$in`, `$gt`, `$gte`, `$lt`, `$lte`), which are not available on `TEXT` fields.
...
FACET fields only support `$eq` and `$in` operators. They are primarily used with the [`$facet` aggregation](/redis/search/aggregations#facet) to build category trees and faceted navigation.
