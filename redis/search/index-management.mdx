...
When you create a search index, Upstash Redis builds an optimized lookup structure for the schema you specify. This allows queries to find matching documents in milliseconds, regardless of dataset size. The index automatically stays in sync with your data: keys matching an index prefix and entries in an indexed stream are added and removed as the underlying data changes.

You define an index once by specifying:

- A **name** to identify the index
- One or more **prefixes** that determine which keys to track, or the exact key of a stream
...
The [`SEARCH.CREATE`](/redis/commands/search/search-create) command creates an index identified by a name, which must be a unique key in Redis. Each index works with a single data type: JSON, hash, string, or stream.
...
- For **stream** indices, each entry in one exact stream is a separate document. Stream fields are
  flat, and query results use the entry ID as the document ID. See
  [Searching Streams](/redis/search/streams) for setup, schema conversion, and lifecycle behavior.
...
By default, creating an index scans and indexes all existing keys matching its type and prefixes, or
all existing entries in its bound stream.
...
the current matching keys or stream entries.
...
| ---------- | ---------------------------------------- |
| `name`     | Index name                               |
| `type`     | Data type (`STRING`, `HASH`, `JSON`, or `STREAM`) |
| `prefixes` | Tracked key prefixes, or the bound key for a stream index |
| `language` | Stemming language                        |
| `schema`   | Field definitions with types and options |

### Listing Indexes

[`SEARCH.LISTINDEXES`](/redis/commands/search/search-listindexes) lists all search indexes in the database, reporting each index's `name` and backing `type` (`HASH`, `JSON`, `STRING`, or `STREAM`). Optionally filter names with `MATCH` and paginate with `LIMIT` and `OFFSET`.
...
```bash
SEARCH.LISTINDEXES
# Returns: [["name", "productIdx", "type", "HASH"], ["name", "eventIdx", "type", "STREAM"]]
