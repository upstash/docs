...
## Does Upstash provide authentication for my application's users?

No. Upstash secures access to your database — the credentials and REST tokens described in [Security](/redis/features/security), [REST API](/redis/features/restapi) and the [Shared Responsibility Model](/redis/help/shared-responsibility-model). Account-level user authentication and authorization (sign-up, sessions, per-user access control) is a customer responsibility that you implement in your application.

## How should I store ordered items with a priority, status, and timestamps?

Redis data modeling for that shape is covered in [Redis Queue](/redis/tutorials/redis_queue) and the [Sorted Set](/redis/commands/sorted-set/overview) command reference. In short: a sorted set stores members ordered by a numeric score (for example a `runAt` or priority-adjusted timestamp), a hash holds the item's mutable status fields, and a Lua script or a conditional command such as [`DELEX`](/redis/commands/string/delex) performs the status transition atomically so concurrent writers cannot race.
...
Free tier databases are archived after a minimum of 30 days of inactivity. Users get several warning emails prior to this operation. Archival means backing up user data and removing the database instance.
