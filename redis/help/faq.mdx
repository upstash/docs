...
## Can I use multiple databases (SELECT / db index)?

No. Upstash exposes a single logical database, so `0` is the only valid index and `SELECT` with any other index returns an error. The command is accepted so that clients which issue `SELECT 0` while setting up a connection keep working. To separate concerns inside one database, use key prefixes instead of numbered databases. See [`SELECT`](/redis/commands/connection/select) for details.

## Are there restricted or unsupported commands?

Yes, depending on the protocol. Over the REST API, for example, connection commands are limited to `PING` and `ECHO`. Blocking commands, `WATCH`/`UNWATCH`/`DISCARD` and Cluster are not supported. See [Redis® API Compatibility](/redis/overall/compatibility) for the full list.
