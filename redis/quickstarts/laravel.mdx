...
Create a Redis database using [Upstash Console](https://console.upstash.com). Go to the **Connect to your database** section and click on **PHP** and then **Laravel**. Copy those values into your .env file. `REDIS_PASSWORD` takes the **Token** shown in the console, which is also your database's password. Note the `tls://` prefix on the endpoint, Upstash requires TLS connections:
