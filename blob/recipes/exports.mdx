...
description: "A background job builds the file, and the download link stops working after a day."
---

<Frame>
  <img className="block dark:hidden" src="/img/blob/exports-light.gif" alt="Click Export, wait while the job builds the file, click Download CSV, then see the link expire a day later" />
  <img className="hidden dark:block" src="/img/blob/exports-dark.gif" alt="Click Export, wait while the job builds the file, click Download CSV, then see the link expire a day later" />
</Frame>
...
Five minutes is the life of the link, not the life of the export. The row's `expiresAt` says whether the export still exists, and the route checks it before signing. To mail the export, mail this route's URL: a signed link lives at most 10 minutes, and works for anyone holding it, with no ownership check.
