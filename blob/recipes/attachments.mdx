...
`uploadRoute()` is the route form that takes an `input` schema and a typed `state`. Put ids in `state`, never secrets: it travels through the browser. `file.name` is the original filename, and this callback is the only place it exists, so store it if you want to show it later.
...
The route declares a schema, so `input` is required here and a missing `threadId` does not compile. Three files upload at a time and the rest queue.
...
Delete the object first, then the row. A failure then leaves a harmless orphan rather than a link that 404s, and `del` is safe to run again.
...
Deleting a whole thread is the same two steps in bulk. The rows name every path, so there is still nothing to list:
