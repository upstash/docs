...
description: "Several files per thread, uploaded from the browser as multipart, one row per attachment."
---

<Frame>
  <img className="block dark:hidden" src="/img/blob/attachments-light.gif" alt="Click Attach, pick two files, watch each row upload, post the reply with both attached, then remove one" />
  <img className="hidden dark:block" src="/img/blob/attachments-dark.gif" alt="Click Attach, pick two files, watch each row upload, post the reply with both attached, then remove one" />
</Frame>
