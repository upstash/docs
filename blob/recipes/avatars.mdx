...
A throw out of `onUploadComplete` deletes the picture that just landed, so the `catch` turns a database failure into a message the user can retry from, rather than a raw driver error. [Upload handler](/blob/uploads/upload-handler#onuploadcomplete) has the reasoning. A refusal costs more here than on a unique path: the old picture was already overwritten, so the user is left with no picture rather than the previous one.

`metadata` is written onto the object itself, so `owner` can be read back later with `bucket.info()`. An id that only the completion callback needs can travel in `state` instead, as in [Product images](/blob/recipes/product-images).
