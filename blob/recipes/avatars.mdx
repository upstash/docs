...
The bytes go straight from the browser to storage, and your server only authorizes the upload. If you have not created a bucket yet, start with the [Quickstart](/blob/overall/quickstart).
...
export const uploads = uploadHandler({
  ...
  onUploadComplete: async ({ metadata, versionedUrl }) => {
    try {
      ...
      // A throw here deletes the picture that just landed. Log instead.
