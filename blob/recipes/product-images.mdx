...
`sortOrder` is a timestamp rather than a count, so three files uploading at once, or one completion request retried, cannot land on the same number. A throw out of `onUploadComplete` deletes the image that just landed, so the `catch` turns a database failure into a message the user can retry from, rather than a raw driver error. [Upload handler](/blob/uploads/upload-handler#onuploadcomplete) has the reasoning.

`state` reaches only `onUploadComplete`, which is all the product id needs. `metadata` would also write it onto the object, for a cron or a later `bucket.info()` to read back; [Profile pictures](/blob/recipes/avatars) uses that form.
...
Deleting a product is the same thing in bulk. The rows are the only thing that knows the paths, so read them first, delete the objects, then drop the rows. If it fails partway, run it again: `del` treats a missing object as success.
