---
title: "Video Uploads"
description: "Multi-gigabyte video uploaded straight from the browser, with progress and resume."
---

<Frame>
  <img className="block dark:hidden" src="/img/blob/video-light.gif" alt="Pick a 1.2 GB video, watch four parts upload at once, pause and resume, close the tab and pick the same file to continue from the parts that landed, then play it back" />
  <img className="hidden dark:block" src="/img/blob/video-dark.gif" alt="Pick a 1.2 GB video, watch four parts upload at once, pause and resume, close the tab and pick the same file to continue from the parts that landed, then play it back" />
</Frame>
...
For videos that must not be watchable by anyone holding the link, use a private bucket and sign each play with `signedReadUrl` at the full `'10m'`. The player keeps fetching ranges from that one link, and no link lives longer, so a longer video needs a fresh link before then. See [Private documents](/blob/recipes/private-documents) for that shape.
