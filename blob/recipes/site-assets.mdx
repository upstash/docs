...
`uniquePath` gives every upload its own object, so re-uploading a file called `hero.png` never replaces last month's `hero.png`. Because the path never repeats, `cache: 'immutable'` needs no invalidation at all. A throw out of `onUploadComplete` deletes the file that just landed, so the `catch` turns a database failure into a message the user can retry from, rather than a raw driver error. [Upload handler](/blob/uploads/upload-handler#onuploadcomplete) has the reasoning.

`metadata` is written onto the object, so `uploader` can be read back later with `bucket.info()`. An id that only the completion callback needs can travel in `state` instead, as in [Product images](/blob/recipes/product-images).
