...
Use a public bucket, since every one of these files is meant to be linked from a page. If you have not set up one yet, start with the [Quickstart](/blob/overall/quickstart).
...
`uniquePath` gives every upload its own object, so re-uploading a file called `hero.png` never replaces last month's `hero.png`. Because the path never repeats, `cache: 'immutable'` needs no invalidation at all. The callbacks are covered in full in [Upload handler](/blob/uploads/upload-handler).
...
`lib/upload-hooks.ts` is the same two lines as in the Quickstart.
...
For CMS media, delete the row first. The page stops linking the file immediately, and a failure on the second step leaves an object nobody references rather than a broken image.
