---
title: "Reading"
description: "get, info, exists and list, public URLs, and signed read URLs on a private bucket."
---
...
| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `expiresIn` | `Duration` | `'5m'` | How long to ask for, at most `'10m'`. `'2m'`, `'10m'`, or a bare number of seconds. A longer ask gets 10 minutes. |
...
`expiresIn` is what you asked for. `expiresAt` is what you got, and it can be sooner, because no link lives longer than 10 minutes. Cache the link until `expiresAt`, never until a deadline you compute yourself. This applies to `signedUploadUrl` too. The reason is on [How signing works](/blob/reference/signing#how-long-a-presigned-url-lives).
