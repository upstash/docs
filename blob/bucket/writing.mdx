...
Slashes define directories wherever they appear, in the template or in an interpolated value, so a prefix can be interpolated as is. `uniquePath` also takes a plain string. Like every path, the result may not contain `.` or `..` segments; that is checked when the path is used.
