...
Slashes in the template literal define directories. Each interpolated value must stay within one path segment. Values containing a forward slash, a backslash, or control characters, or equal to `.` or `..`, throw `TypeError` instead of being rewritten. The assembled path is also checked for `.` and `..` segments.
...
uniquePath`chat/${"../admin/x.png"}`           // throws TypeError
uniquePath`chat/${"a/b"}`                      // throws TypeError
```
