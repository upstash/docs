---
title: "Writing"
description: "put, unique paths, metadata, conditional writes, copy and move, and signed upload URLs."
---
...
const path = uniquePath`users/${userId}/${filename}`
// 'users/Alice_123/Q3 Report-3xK9mBqR.pdf'
```

`uniquePath` adds one random suffix to the final filename while preserving the rest of the path. Case, underscores, spaces, punctuation, and Unicode stay exactly as supplied. User IDs such as `Alice_123` remain `Alice_123`; values are not lowercased, slugged, truncated, or Unicode-normalized.

Slashes define directories wherever they appear, in the template or in an interpolated value, so a prefix can be interpolated as is. `uniquePath` also takes a plain string. Like every path, the result may not contain `.` or `..` segments. `uniquePath` throws an `invalid_input` error for one, so a browser filename like `../x.png` is refused before your code records the path.

```ts
uniquePath`chat/${"Alice_123"}/${"Photo.PNG"}` // 'chat/Alice_123/Photo-9fQ2mAe7.PNG'
uniquePath`${"Q3 Report (final).pdf"}`         // 'Q3 Report (final)-7hTbN2xY.pdf'
uniquePath`${"café.png"}`                      // 'café-Wm4pQ8dK.png'
uniquePath`${"uploads/"}${"Alice_123"}/a.png`  // 'uploads/Alice_123/a-Zc5rT1vB.png'
uniquePath(`uploads/${"Alice_123"}/a.png`)     // 'uploads/Alice_123/a-Hq8mW3nK.png'
```

The suffix has 8 random characters and goes before the final extension when it is 1 to 8 ASCII letters or digits, preserving the extension's case. Otherwise it goes at the end. A leading dot stays part of the filename: `.env` becomes `.env-<random>`, while `.env.local` becomes `.env-<random>.local`. An empty final filename uses `file` as its stem. The SDK handles URL encoding when it sends the path to storage.

Store the returned path with the original owner ID and use that exact path for reads and deletion. The helper creates a unique key; ownership checks still belong in your application. See [Private documents](/blob/recipes/private-documents).
