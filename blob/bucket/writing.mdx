...
| `contentTypes` | `string[]` | An allow list such as `["image/*", "application/pdf"]`. The declared type must be in it, and the body's leading bytes must not contradict the declared type. A refusal is `content_type_not_allowed` and nothing is written. |
| `maxBytes` | `Size` | Refuses a body over this size with `too_large`. Also caps how much of an unknown-length stream is buffered. |
| `cache` | `CacheOption` | The `Cache-Control` this object is stored with. Overrides the bucket default. |
| `metadata` | `Record<string, string>` | Custom key-value pairs stored with the object. Keys come back lowercased, values must be printable ASCII. |
| `size` | `number` | The exact length in bytes, for a stream whose size is not otherwise known. |
| `overwrite` | `boolean` | `false` refuses the write if something is already at the path. |
| `ifUnchanged` | `string` | An etag. The write fails with `conflict` if the object changed since you read it. |
| `multipart` | `boolean \| Size` | Bodies over this size go up in parts. Default 16 MB. `true` always, `false` never. |
