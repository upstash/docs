---
title: "Connecting"
description: "Build a Bucket client from environment variables, on Node, on Cloudflare Workers, or through an S3 client."
---
...
The SDK sends its version, runtime and platform as headers on credential and signing requests to Upstash. Turn it off with `UPSTASH_DISABLE_TELEMETRY` in the environment (any value), or `enableTelemetry: false` on the `Bucket`.
...
<Warning>
Do not presign URLs with this client. A presigned URL carries the credential, which reads and writes the whole bucket until it expires. Use [`signedReadUrl`](/blob/bucket/reading#signedreadurl) and [`signedUploadUrl`](/blob/bucket/writing#signed-upload-urls), which Upstash signs for one object; see [Who signs a URL](/blob/reference/signing#who-signs-a-url).
</Warning>
