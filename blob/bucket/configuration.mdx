...
| `visibility` | `'public' \| 'private'` | the console setting | `'private'` drops `url` and `versionedUrl` from every record. A bucket that is private in the console is private here regardless. See [Private buckets](/blob/bucket/reading#private-buckets). |
