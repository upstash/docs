---
title: "Constraints"
description: "maxSize, contentTypes and byte sniffing, enforced on your server before anything is signed."
---
...
constraints: { maxSize: "32MiB" } // 33,554,432 bytes
constraints: { maxSize: 4096 }    // a bare number is bytes
```

`maxSize` takes a [Size](/blob/reference/types#size), always in bytes. `MB` uses powers of 1,000; `MiB` uses powers of 1,024. Units ignore case. To accept a 32 MiB file, set `maxSize: "32MiB"` or `maxSize: 33_554_432`. `"32mb"` means 32,000,000 bytes and is too small for that file.

Refusal messages display decimal byte units, or exact bytes when the file and the limit would round to the same text:
