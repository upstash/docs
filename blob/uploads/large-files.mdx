---
title: "Large Files"
description: "Multipart uploads from the browser, with progress, pause, resume, retries and cancel."
---
...
The default is **16 MB decimal**, 16,000,000 bytes. A 16,000,000 byte body is a single PUT; 16,000,001 is multipart. [Size options](/blob/reference/types#size) accept decimal units such as `MB` and explicit binary units such as `MiB`, always in bytes. `bucket.put()` on your server and a direct browser upload use the same threshold.
...
For a 32 MiB file, use an explicit binary unit or its byte count. `"32MB"` is only 32,000,000 bytes, below the 33,554,432-byte file size.

```ts lib/uploads.ts
export const uploads = uploadHandler({
  constraints: { maxSize: "32MiB" }, // also accepts 33_554_432
  onBeforeUpload: ({ file }) => ({ path: uniquePath`uploads/${file.name}` }),
})
```

The default 16 MB threshold makes this a multipart upload. `maxSize` sets the file limit; `multipart` chooses when to split an accepted file into parts.
