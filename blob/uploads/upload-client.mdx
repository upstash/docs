...
| Default | Description |
| --- | --- |
| `headers` | A function returning headers to send to your route. Re-read on every request. |
| `concurrency` | How many files upload at once. Default 3. |
| `endpoint` | Where the handler is mounted. Default `/api/upload`. |
| `onError` | Runs for every failed upload. |
...
| Option | Description |
| --- | --- |
| `field` | The form field the file is sent under. Default `'file'`. Must match what your route reads. |
| `headers` | A function returning headers to send with the request. |
