...
export const GET = async () => {
  ...
  for (const row of rows) {
    try {
      ...
      if (info.metadata.rowid === row.id) {
        await bucket.del(row.path)
        deleted++
      }
    } catch (e) {
      // Nothing stands at the path: the PUT never finished, or the abort above just
      // cleared the parts. Either way the row is the leftover.
      if (!(BlobError.is(e) && e.code === "not_found")) throw e
    }
    await db.uploads.delete(row.id)
  }

  return Response.json({ aborted: aborted.length, swept: rows.length, deleted })
}
```
...
- **Abort first, one window.** A route without `multipart: true` still uses parts for a file over the threshold, so a pending row can belong to a paused multipart upload. To `info()` that looks exactly like a PUT that never finished: both throw `not_found`. Running the abort first with the same window means that by the time the row sweep sees `not_found`, either nothing was ever stored or the parts are already gone. A row sweep on a shorter window would delete rows out from under uploads that can still resume, and the resumed upload would then complete against a row that no longer exists.
