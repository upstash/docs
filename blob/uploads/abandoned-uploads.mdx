...
| Who cleans it up | you, with a pending row or a list sweep | `bucket.abortStaleMultipartUploads()` on a cron |

Both halves need a cron. The multipart half is one call. The single-PUT half needs your app's help, because the SDK cannot tell an abandoned object from a completed one by looking at the bucket. Only your own records can.
...
The calls it is built from are on [Deleting](/blob/bucket/deleting#incomplete-multipart-uploads).

---

## Single PUT: the pending row

If you keep the single-PUT path, write a row before the bytes, flip it after them, and sweep what never flipped.
...
<Note>
  `uploadId` first reaches your code in `onUploadComplete`, so the example mints its own row id in `onBeforeUpload`. It travels as `metadata`, which the cron reads back with `info()`, and as `state`, which reaches `onUploadComplete` typed. Metadata keys come back lowercased, so the example writes `rowid`. See [Metadata](/blob/bucket/writing#metadata).
</Note>
...
export const GET = async () => {
  ...
  for (const row of rows) {
    try {
      ...
      // info() returns metadata, so this says the object standing at the path is the
...
- **Verify before deleting.** A row's path may hold a different upload's object by the time the cron runs. `bucket.info(path)` returns the metadata, which says whether the object is the one this row reserved. `info()` throws `not_found` when nothing was stored, which is why the example catches it.
- **Pick a grace window longer than your longest upload.** A window measured in minutes deletes objects out from under uploads that are still running.
