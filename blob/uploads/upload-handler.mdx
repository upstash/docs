...
description: "Mount the upload route, authorize the upload in onBeforeUpload, and record the file in onUploadComplete."
---

`uploadHandler` is the server side of a direct browser upload. It authorizes the upload, signs it, and records what landed. The bytes go from the browser straight to storage and never touch your server, so uploads are not limited by your platform's request body cap, the 4.5MB one on Vercel or the 6MB one on AWS Lambda.
...
uniquePath`chat/${"Thread_42"}/${"Holiday Pic.PNG"}`
// chat/Thread_42/Holiday Pic-7Kd2mQ9x.PNG
```

`path` is required, and may not contain `.` or `..` segments. `uniquePath` preserves IDs and filenames, including case, underscores, spaces, and Unicode, and adds one random suffix to the final filename. Interpolated values containing directory separators or control characters, or equal to `.` or `..`, are rejected instead of rewritten. Put directory separators in the template literal. The rules are on [Writing](/blob/bucket/writing#uniquepath).
