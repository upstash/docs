...
`path` is required, and may not contain `.` or `..` segments. `uniquePath` preserves IDs and filenames, including case, underscores, spaces, and Unicode, and adds one random suffix to the final filename. Slashes may come from the template or from interpolated values, so a prefix can be interpolated directly. The rules are on [Writing](/blob/bucket/writing#uniquepath).
