...
Bad option values are not in this list. An unparseable `'5mbit'`, a missing `token`, a route with no `onBeforeUpload`: those throw a `TypeError` where the option is written, not a `BlobError` per request.
