---
title: "Errors"
description: "BlobError codes, what reaches the browser, and the platform body limits behind a 413."
---
...
| `invalid_input` | 400 | `invalid input` | Arguments the SDK will not accept: metadata outside printable ASCII, a malformed upload request body, `input` that fails the route's schema, a `del` target that is none of the three shapes, a signed URL for a path with an empty segment. |
...
| `forbidden` | 403 | `forbidden` | A completion token that is not valid for this route, or has expired. Also a signed URL Upstash refused for the bucket, such as an upload URL on a read-only bucket; `e.cause` says why. |
| `rate_limited` | 429 | `rate limited` | Storage answered `SlowDown` or `TooManyRequests`, or credential or signing requests are being rate limited. |
...
`e.message` never carries a credential, a token, or an internal path. Every message is assembled from a code, a caller-supplied string, an HTTP status, or the reason Upstash's signing service gave. A suspended or read-only bucket is the exception: that notice is meant for you, not your users, so it is on `e.cause` instead.
...
Vercel caps a serverless request body at 4.5MB, AWS Lambda at 6MB, and Cloudflare at 100MB on the free plan. A file over that cap is refused with a 413 that never reached your route, so it carries no code of its own. The SDK turns it into `too_large` and attaches the cap as a hint:
...
| ---- | ------ | ------- | ---------- |
| `unauthorized` | 401 | The bucket token was rejected, or the bucket is suspended. | Check `UPSTASH_BLOB_TOKEN`. When a signed URL was refused, `e.cause` says why. Nothing retries this. |
