...
| `forbidden` | 403 | `forbidden` | A completion token that is not valid for this route, or has expired. Also a signed upload URL on a read-only bucket. |
...
`e.message` never carries a credential, a token, or an internal path. Every message is assembled from a code, a caller-supplied string, an HTTP status, or Upstash's reason for refusing a malformed signing request.
