...
| `length_required` | 411 | `length required` | `put` of an unknown-length stream with neither `size` nor `maxBytes`. |
...
Keep a proxied route's own `maxBytes` under the platform's cap, so the refusal comes from your code with your wording. A file bigger than the cap needs a direct browser upload instead.
