...
`e.message` never carries a credential, a token, or an internal path. Every message is assembled from a code, a caller-supplied string, an HTTP status, or the reason Upstash gave for refusing to sign a URL.
