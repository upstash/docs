...
```shell
curl https://tops-stingray-7863-eu1-rest-kafka.upstash.io/schema-registry/my-subject/$SUBJECT/versions -u myuser:mypass -d '
{
  "schema": "{
    \"type\": \"record\",
    \"name\": \"myRecord\",
...
Returns all schemas registered under subjects. Note that this endpoint will return same schema multiple times if same schema registered
...
```shell 
curl https://tops-stingray-7863-eu1-rest-kafka.upstash.io/schema-registry/config/schemas -u myuser:mypass
```
...
Returns the schema corresponding the given `$SCHEMA_ID`. 
...
Returns the schema corresponding the given `$SCHEMA_ID`. Additionally unwraps the inner schema field.
...
Returns the only the schema corresponding the given `$SUBJECT` and `$VERSION`. 
VERSION could be an int or string `latest`. 
...
`deleted` is false by default. If set to `true`, the soft-deleted subjects will also be take into account. 
See [Delete Subject](#delete-subject) or [Delete Schema](#delete-schema) for details.

```shell 
curl https://tops-stingray-7863-eu1-rest-kafka.upstash.io/schema-registry/config/subjects -u myuser:mypass
```
...
`deleted` is false by default. If set to `true`, the soft-deleted subject versions will also be take into account. 
See [Delete Subject](#delete-subject) or [Delete Schema](#delete-schema) for details.
...
Deletes the given `$SUBJECT`. Returns the deleted versions as response.
...
`permanent` is false by default. In this case, subject only will be soft-deleted. The corresponding schemas and schema-ids
...
```shell
curl -X DELETE https://tops-stingray-7863-eu1-rest-kafka.upstash.io/schema-registry/subjects/my-subject -u myuser:mypass -d
```

**Success Response:**

Returns the deleted versions as response.
...
Deletes the schema corresponding to the `$SUBJECT` and `$VERSION`. Returns the version as response.
VERSION could be an int or string `latest`. 
...
`permanent` is false by default. In this case, subject-version only will be soft-deleted. The corresponding schema and schema-id
...
```shell
curl -X DELETE https://tops-stingray-7863-eu1-rest-kafka.upstash.io/schema-registry/subjects/my-subject/versions/2 -u myuser:mypass -d
```

**Success Response:**

Returns the deleted version as response.
