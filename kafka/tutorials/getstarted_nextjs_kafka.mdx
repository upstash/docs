...
In this post, we will implement the simplest application where we will
publish messages to Kafka from a Next.js application.

### Project Setup

First, create a Next.js project with:

```
â  kafka-examples git:(master) â npx create-next-app@latest
â What is your project named? â¦ getstarted-nextjs
...
[here](https://docs.upstash.com/kafka). On the cluster page, under the REST API
section, copy the producer code under the tab `Javascript (fetch)`.

### Implementation

Paste the producer code into the `pages/api/hello.js` as below:
