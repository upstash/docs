...
You can further restrict the search by providing a filter based on a specific metadata keys.

Search queries with filters only return documents which have the metadata matching with the filter.

Upstash Search allows you to filter by metadata keys which have the following value types:
...
A filter has a syntax that resembles SQL, which consists of operators on metadata keys and boolean operators
to combine them.

Assuming you have metadata like below:
...
The `equals` operator filters metadata whose values are equal to the given literal.
...
The `not equals` operator filters  metadata whose values are not equal to the given literal.
...
The `less than` operator filters  metadata whose values are less than the given literal.
...
The `less than or equals` operator filters metadata whose values are less than or equal to the given literal.
...
The `greater than` operator filters metadata whose values are greater than the given literal.
...
The `greater than or equals` operator filters metadata whose values are greater than or equal to the given literal.
...
The `glob` operator filters metadata whose values match with the given UNIX glob pattern.
...
The `not glob` operator filters metadata whose values do not match with the given UNIX glob pattern.
...
The `in` operator filters metadata whose values are equal to any of the given literals.
...
The `not in` operator filters metadata whose values are not equal to any of the given literals.
...
The `contains` operator filters metadata whose values contain the given literal.
...
The `not contains` operator filters metadata whose values do not contain the given literal.
...
The `has field` operator filters metadata which have the given JSON field.
...
The `has not field` operator filters metadata which do not have the given JSON field.
