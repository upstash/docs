...
Let's create a new Next.js application with App Router and install `@upstash/search` package.
...
### Create a Route to Upsert Documents
...
export async function POST() {
  await index.upsert([
    {
      id: "film-1",
      content: { title: "Inception", description: "A mind-bending thriller about dreams within dreams." },
      metadata: { genre: "Sci-Fi", year: 2010 },
    },
    {
      id: "film-2",
      content: { title: "The Godfather", description: "A story about a powerful Italian-American crime family." },
      metadata: { genre: "Crime", year: 1972 },
    },
    {
      id: "film-3",
      content: { title: "The Dark Knight", description: "A gritty tale of Batman's fight against the Joker." },
      metadata: { genre: "Action", year: 2008 },
    },
  ]);
  return new Response(JSON.stringify({ message: "Documents upserted successfully" }), {
    status: 200,
...
### Create a Route to Search Documents
...
You can click the `Upsert Data` button to add three films to your database and then use the search bar to make a query.
