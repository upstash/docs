...
1. Node.js and npm (or another package manager) installed.
...
You can start the local server by running:
...
Now you can open the Upstash Workflow dashboard and enable local mode to use the UI while developing locally.
Your workflow runs will appear in the dashboard, where you can monitor and analyze them.

<Frame>
  <img src="/img/qstash-workflow/local-dev.png" />
</Frame>
...
In the root of your project, create a `.env.local` file (or update your existing one) and add the values printed by the QStash dev server:
...
<Tip>
  When preparing your app for production, you will update these values with your production creadentials from the Upstash Workflow dashboard.
</Tip>
...
Each endpoint you define with the SDK represents a separate workflow that Upstash can trigger.
...
Now you can open the Upstash Workflow dashboard to view the logs of your workflow runs and check if any errors occurred.

<Frame>
  <img src="/img/qstash-workflow/run-view.png" />
</Frame>
...
1. **Remove Local Development Settings**: Clean up or conditionally exclude any development-only configuration, such as the local base URL and QStash credentials.

2. **Environment Variables**: Ensure that all required environment variables are configured in your deployment environment with production credentials from the Upstash Workflow dashboard.
...
2. **[Configure Workflow Runs](/workflow/basics/client)**: Learn how to congfigure workflow execution to fit your app's needs.

3. **[Handle Failures](/workflow/howto/failures)**: Understand how to detect, log, and recover from failed workflow runs.