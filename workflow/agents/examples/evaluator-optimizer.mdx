...
<img src="/img/workflow/agents/evaluator-diagram.png" />
...
export const { POST } = serve(async (context) => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    // Generate content
    const generatedResponse = await context.agents.task({ agent: generator, prompt }).run();
...
<img src="/img/workflow/agents/evaluator-optimizer.png" />
