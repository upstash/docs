...
api: "GET https://qstash.upstash.io/v2/workflows/events/runs"
authMethod: "bearer"
---

You can fetch details about workflow runs, including their state, completed and in-progress steps, and step details.

The retention duration for completed workflow runs depends on your quota. Please check the [pricing](https://upstash.com/pricing/qstash) page for details.

<Note>
If you are using custom workflow run IDs, this endpoint may return multiple runs when filtering by run ID.
...
<ParamField query="fromDate" type="number">
  Filter workflow runs by starting date, in milliseconds (Unix timestamp). This
  is inclusive.
</ParamField>

<ParamField query="toDate" type="number">
  Filter events by ending date, in milliseconds (Unix timestamp). This is
  inclusive.
</ParamField>

<ParamField query="count" type="number">
  The number of events to return. Default and max is 10.
</ParamField>
...
<RequestExample>
  ```sh curl
  curl https://qstash.upstash.io/v2/workflows/events/runs \
  ...
  const runs = await client.runs({ workflowRunId: "<WORKFLOW_RUN_ID>"});

  // Filter by workflow server url
  const runs = await client.runs({ workflowUrl: "<WORKFLOW_RUN_ID>"});

  // Filter by state
  const runs = await client.runs({ state: "RUN_SUCCESS"});
  ```

  ```javascript Node
  const response = await fetch("https://qstash.upstash.io/v2/workflows/events/runs", {
    headers: {
  ...
  response = requests.get(
    'https://qstash.upstash.io/v2/workflows/events/runs',
    headers=headers
  )
  ```

  ```go Go
  req, err := http.NewRequest("GET", "https://qstash.upstash.io/v2/workflows/events/runs", nil)
  if err != nil {
    log.Fatal(err)
  }
