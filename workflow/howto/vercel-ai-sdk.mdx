...
<Info>
The `maxSteps` parameter controls how many times the model can call tools. For each tool call, the model needs to be called twice: once to generate the tool call and again to process the tool result. Set `maxSteps` greater than 1 to enable tool usage - for example, `maxSteps: 2` allows one complete tool interaction. See the [Multi-Step Calls section in Vercel Docs](https://sdk.vercel.ai/docs/ai-sdk-core/tools-and-tool-calling#multi-step-calls) for more details.
</Info>
