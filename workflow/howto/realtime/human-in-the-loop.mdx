...
export type RealtimeEvents = {
  "workflow.update": 
    | { type: "runStart"; workflowRunId: string; timestamp: number }
    | { type: "runFinish"; workflowRunId: string; timestamp: number; status: "success" | "failed"; error?: string }
    | { type: "stepStart"; workflowRunId: string; stepName: string; timestamp: number }
    | { type: "stepFinish"; workflowRunId: string; stepName: string; timestamp: number; result?: unknown }
    | { type: "stepFail"; workflowRunId: string; stepName: string; timestamp: number; error: string }
    | { type: "waitingForInput"; workflowRunId: string; eventId: string; message: string; timestamp: number }
    | { type: "inputResolved"; workflowRunId: string; eventId: string; timestamp: number };
};
```
...
export const { POST } = serve<WorkflowPayload>(
  async (context) => {
    ...
    await context.run("process-approval", async () => {
      ...
      const result = {
        status: eventData.approved ? "approved" : "rejected",
  ...
  return (
    <div>
      <button onClick={trigger} disabled={isTriggering}>
        {isTriggering ? "Starting..." : "Trigger Workflow"}
      </button>

      {runStatus && <div>Status: {runStatus.status}</div>}
