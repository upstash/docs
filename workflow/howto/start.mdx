...
<Warning>
    If you’ve secured your endpoint with signing keys, only the `trigger` methid will work. Direct calls to the endpoint (e.g., via `curl` or `fetch`) will not be possible since `Upstash-Signature` header is missing.
