...
- [context.call](/workflow) – lets you run external HTTP requests under a separate key, so you can throttle third‑party API calls independently of your workflow logic.
- [context.invoke](/workflow) – starts a new workflow run with its own flow control configuration. This allows the invoked workflow to run under different limits than the parent workflow, giving you more precise control.
...
See [Advanced Per-Step Configuration](/workflow) for more details.
</Note>
