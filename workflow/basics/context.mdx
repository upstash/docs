...
`context.fail` allows you to fail the current workflow on will 
without going into retry cycle.
...
export const { POST } = serve<{ topic: string }>(async (context) => {
  ...
  if (result.fail) {
    await context.fail() // fail the workflow run
  }
})
```

### Distinction between `context.cancel` and `context.fail`
`context.cancel`:
- run will be labeled as canceled.
- failure function will not run and a dlq entry won't be created

`context.fail`:
- run will be labeled as failed
- failure function will run, dlq entry will be created
