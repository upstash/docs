...
A limitation of `context.call` is that it does not support using `localhost` or certain QStash endpoints (such as [the publish endpoint](/qstash/api/publish)) as the destination URL. Attempting to send a request to these endpoints will result in `context.call` failing.
