...
<CodeGroup>
```typescript Serial execution (JS)
import { serve } from "@upstash/workflow/nextjs";
...
```typescript Parallel execution (JS)
import { serve } from "@upstash/workflow/nextjs"
...
const { status, body } = await context.api.anthropic.call("Call Anthropic", {
...
If you want to overwrite the expected return type or the body type, you can do it by passing the types like this:

```typescript
type BodyType = { ... }
type ReturnType = { ... }

await context.api.openai.call<ReturnType, BodyType>( ... )
```
