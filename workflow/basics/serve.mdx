...
2. **Options**: configuration options that control the workflow behavior.
...
Options configure how the workflow executes. These settings apply to every run of the workflow.
...
<ParamField path="failureFunction" type="string">
    ...
    The failure function itself also respects the retry configuration.
    ...
    <Accordion title="Advanced failureUrl option">
        ...
        <ParamField path="failureUrl" type="string">
            ...
            For more details, see [failureUrl](/workflow/features/failureFunction/advanced).
...
<ParamField path="retryDelay" type="string">
    ...
    You can use the `retried` variable—which starts at 0 for the first retry—to compute a dynamic delay.
