...
To set the rate that can the envrionment that workflow is deployed per second, you can use the `rate` parameter.
...
export const { POST } = serve<string>(
  async (context) => { ... },
  {
    flowControl: {key: "aFlowControlKey",  ratePerSecond: 10}
  }
);
```
...
To set the how many workflow runs can be executed in parallel, you can use the `parallelism` parameter.

By default, there is no parallelism limit.
...
export const { POST } = serve<string>(
  async (context) => { ... },
  {
    flowControl: {key: "aFlowControlKey", parallelism: 3}
  }
);
```
