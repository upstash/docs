...
without echo. If you pick the Claude subscription option and Claude Code is installed on your
machine, setup runs `claude setup-token` and stores the token it produces, so a Pro or Max plan works
without a pay-per-token API key.

That is all the setup there is. The two sections below describe what setup wrote, for when you want
...
rejects anything over 100 MB, so raising `maxUploadBytes` will not get you past it. Exclude the heavy
paths instead:
