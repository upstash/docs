...
### Combining with AI SDK

If you're running an AI SDK (like Vercel AI SDK) inside a box, you can use both approaches:

```tsx
const box = await Box.create({
  runtime: "node",
  // Agent uses managed key (handled by Upstash)
  agent: {
    provider: Agent.ClaudeCode,
    model: ClaudeCode.Sonnet_4_6,
    apiKey: BoxApiKey.UpstashKey,
  },
  // Inject Stripe key for user code — never visible in the container
  attachHeaders: {
    "api.stripe.com": {
      Authorization: "Bearer sk_live_...",
    },
