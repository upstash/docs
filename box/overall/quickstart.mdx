...
<Note>
  Upstash Box is standalone. You only need a Box API key — no Redis database or
  any other Upstash service is required. Redis appears in integrations such as
  [AgentKit for Vercel Eve](/box/guides/eve-setup), where it is used only for
  optional features like the template registry.
</Note>
