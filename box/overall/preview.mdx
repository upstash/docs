...
```typescript
// First preview
const preview1 = await box.getPreviewUrl(3000)

// Second preview overwrites the first one
const preview2 = await box.getPreviewUrl(3000, { bearerToken: true })
...
const frontendPreview = await box.getPreviewUrl(3000)
const apiPreview = await box.getPreviewUrl(8080)
