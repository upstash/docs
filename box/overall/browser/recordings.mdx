...
Save a completed recording as a regular MP4 file:

<CodeGroup>
```typescript box.ts
const path = await box.browser.recordings.download(saved.id, {
  path: "./session.mp4",
})
```

```python box.py
path = box.browser.recordings.download(saved.id, path="./session.mp4")
```
</CodeGroup>

When `path` is omitted, the file is written to the working directory as `box-recording-<id>.mp4`. Recordings captured before MP4 support download as raw MPEG-TS with a `.ts` extension instead; check `mp4SizeBytes` (`mp4_size_bytes` in Python) on the recording's metadata if you need to know in advance.
