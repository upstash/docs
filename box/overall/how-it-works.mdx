...
const [security, quality, architecture] = await Promise.all([
  Box.create({ runtime: "node", agent: { harness: Agent.ClaudeCode, model: ClaudeCode.Sonnet_4_6 } }),
  Box.create({ runtime: "node", agent: { harness: Agent.ClaudeCode, model: ClaudeCode.Sonnet_4_6 } }),
  Box.create({ runtime: "node", agent: { harness: Agent.ClaudeCode, model: ClaudeCode.Sonnet_4_6 } }),
])
...
const jury = await Box.create({
  runtime: "node",
  agent: { harness: Agent.ClaudeCode, model: ClaudeCode.Sonnet_4_6 },
...
const boxes = await Promise.all(
  models.map((model) => Box.create({ runtime: "node", agent: { harness: inferDefaultProvider(model), model } })),
)
