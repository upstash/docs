...
Box is a standalone product: it runs on its own API key and requires no Redis database or other Upstash service. Any Redis connection you see in Box integrations is only for optional features such as an [AgentKit](/redis/sdks/agentkit/eve) template registry.
