...
- **Agent configuration**: the agent provider, model, and API key settings.
...
The restored box starts with the same workspace state from the snapshot. Lifecycle settings such as keep-alive should be configured on the new box according to the client or surface you are using.
...
const box = await Box.create({
  runtime: "node",
  agent: { provider: Agent.ClaudeCode, model: ClaudeCode.Opus_4_6, apiKey: process.env.ANTHROPIC_API_KEY },
