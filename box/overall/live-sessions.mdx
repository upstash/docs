...
code = session.wait(300)  # seconds to wait
...
<CodeGroup>
```typescript box.ts
session.terminate(5000) // SIGTERM, then SIGKILL after 5s
await session.wait()
```
...
<Note>
  Only the first `terminate` starts the sequence. Later calls are ignored, so the grace period cannot be changed once it is running. Send `KILL` to stop the process immediately instead.
</Note>
...
Both SDKs expose this as a context manager, which stops the process on the way out even if your code raises.
...
const timer = setTimeout(() => build.terminate(5000), 60000)
