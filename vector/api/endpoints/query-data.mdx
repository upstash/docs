---
title: "Query Data"
description: "This endpoint queries the raw text data over the existing vectors in the index after embedding it."
api: "POST https://{endpoint}/query-data/{namespace}"
authMethod: "bearer"
---

## Path

<ParamField path="namespace" type="string">
  The namespace to use. Leave empty to use the default namespace. For more
  information on namespaces see the [dedicated
  section](/vector/features/namespaces).
</ParamField>

## Payload

<ParamField body="data" name="data" type="string" required>
  The raw text data to embed and query.
</ParamField>
<ParamField body="topK" type="number" required>
  The total number of the vectors that you want to receive as a query result.
  The response will be sorted based on the distance metric score, and `topK`
  vectors will be returned.
</ParamField>
<ParamField body="includeMetadata" type="boolean">
  Whether to include the metadata of the vectors in the response. Setting this
  `true` would be the best practice, since it will make it easier to identify
  the vectors.
</ParamField>
<ParamField body="includeVectors" type="boolean">
  Whether to include the vector data of the resulting vectors.
</ParamField>
<ParamField body="filter" type="string">
  Metadata filter to apply (See [Metadata
  Filtering](/vector/features/filtering))
</ParamField>

## Response

<Note>
  The score is normalized to always be between 0 and 1. The closer the score is
  to 1, the more similar the vector is to the query vector. This does not depend
  on the distance metric you use.
</Note>

<ResponseField name="Response" type="Array">
  <Expandable defaultOpen="true">
    <ResponseField name="id" type="string" required>
      The ID of the resulting vector.
    </ResponseField>
    <ResponseField name="score" type="number" required>
      The score of the vector data, calculated based on the distance metric of
      your index.
    </ResponseField>
    <ResponseField name="vector" type="number[]">
      The resulting vector data.
    </ResponseField>
    <ResponseField name="metadata" type="Object">
      Whether to include the vector values of the resulting vector objects.
    </ResponseField>
  </Expandable>
</ResponseField>

<RequestExample>

```sh curl
curl https://better-dodo-20522-us1-vector.upstash.io/query-data \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -d '{
    "data": "What is Upstash?",
    "topK": 2,
    "includeVectors": true,
    "includeMetadata": true,
    "filter": "key = \"value\""
  }'
```

```js Node
const url = "https://better-dodo-20522-us1-vector.upstash.io/query-data"; // Replace with your index endpoint.
const token = "YOUR_TOKEN"; // Replace with your actual token
const data = {
  data: "What is Upstash?",
  topK: 2,
  includeMetadata: true,
  includeVectors: true,
  filter: "key = 'value'",
};

fetch(url, {
  method: "POST",
  headers: {
    Authorization: `Bearer ${token}`,
    "Content-Type": "application/json",
  },
  body: JSON.stringify(data),
})
  .then((response) => response.json())
  .then((data) => console.log(data))
  .catch((error) => console.error("Error:", error));
```

```python Python
import requests
import json

url = "https://better-dodo-20522-us1-vector.upstash.io/query-data" # Replace with your index endpoint.
token = "YOUR_TOKEN" # Replace with your actual token
headers = {
    "Authorization": f"Bearer {token}",
    "Content-Type": "application/json"
}
data = {
    "data": "What is Upstash?",
    "topK": 2,
    "includeMetadata": True,
    "includeVectors": True,
    "filter": "key = 'value'"
}

response = requests.post(url, headers=headers, data=json.dumps(data))
print(response.json())
```

</RequestExample>

<ResponseExample>
```json 200 OK
{
  "result" : [ 
    {
      "id" : "id12",
      "score" : 1.0,
      "vector" : [ 0.42, 0.63, 0.05, 0.72, ..., 0.83, 0.49, 0.6, 0.48 ],
      "metadata" : {"key":"value"}
    }, 
    {
      "id" : "id11",
      "score" : 0.99996454,
      "vector" : [ 0.44, 0.63, 0.05, 0.72, ..., 0.83, 0.49, 0.6, 0.48 ],
      "metadata" : {"key":"value"}
    } 
  ]
}
```
</ResponseExample>
