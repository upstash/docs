...
export default {
	async fetch(request, env, ctx): Promise<Response> {
		const ragChat = new RAGChat({
			redis: new Redis({ token: env.UPSTASH_REDIS_REST_TOKEN, url: env.UPSTASH_REDIS_REST_URL }),
			vector: new Index({ token: env.UPSTASH_VECTOR_REST_TOKEN, url: env.UPSTASH_VECTOR_REST_URL, cache: false }),
