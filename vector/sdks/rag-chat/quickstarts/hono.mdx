...
app.get('/', async (c) => {
  const ragChat = new RAGChat({
    redis: new Redis({ token: c.env.UPSTASH_REDIS_REST_TOKEN, url: c.env.UPSTASH_REDIS_REST_URL }),
    vector: new Index({ token: c.env.UPSTASH_VECTOR_REST_TOKEN, url: c.env.UPSTASH_VECTOR_REST_URL, cache: false }),
