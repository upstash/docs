...
| Result | Explanation |
| ------ | ----------- |
| Hit    | Case when identifier `ip1` is found in the runtime cache |
| Miss   | The first time the rate limit method is called with identifier `ip1` during the same process |
