...
Timeout value in seconds to use when calling a schedule URL ([See `Upstash-Timeout` in Create Schedule page](http://localhost:3000/qstash/api/schedules/create)).
...
// check if paused
const result = await client.schedules.get(createResult.scheduleId);
